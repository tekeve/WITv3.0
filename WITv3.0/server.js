...
function startServer(client) {
    ...
    app.get('/callback', async (req, res) => {
        ...
        if (!discordId) {
            console.error('Invalid or expired state received in ESI callback.');
        ...
        try {
            ...
            console.log(`Successfully authenticated character ${CharacterName} for Discord user ${discordId}.`);
            ...
            console.error('Error during ESI OAuth callback:', errorMessage);
            res.status(500).send('<h1>Authentication Failed</h1><p>An error occurred while processing your request. Please try again.</p>');
        }
    });

    app.listen(port, () => {
        console.log(`ESI auth callback server listening on port ${port}`);
    });
}
