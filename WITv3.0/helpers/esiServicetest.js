...
module.exports = {
    ...
    get: async ({ endpoint, params, headers, caller }) => {
        // --- START OF TEST MODIFICATION ---
        // Simulate an ESI downtime error specifically for the incursions endpoint
        if (endpoint === '/incursions/') {
            logger.warn('--- !!! ESI SIMULATION: Forcing a 503 Service Unavailable error for /incursions/ endpoint. !!! ---');
            const error = new Error('Request failed with status code 503');
            error.response = {
                status: 503,
                data: { error: 'Service Unavailable (Simulated)' }
            };
            throw error;
        }
